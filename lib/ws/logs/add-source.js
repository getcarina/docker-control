...
module.exports = (socket, data) => {
  ...
  version.get({
    sessionId: socket.locals.userInfo.sessionId,
    cluster: socket.locals.userInfo.cluster,
    creds: socket.locals.credentials
  })
  .then((ApiVersion) => {
    // Form the URL to get the container logs
    var requestPath = '/v{ApiVersion}/containers/' + encodeURIComponent(data.container) + '/logs?stdout=1&stderr=1&tail=40&follow=1';
    requestPath = requestPath.replace('{ApiVersion}', ApiVersion);
